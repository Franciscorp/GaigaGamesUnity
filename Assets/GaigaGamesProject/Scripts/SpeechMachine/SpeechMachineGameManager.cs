...
public class SpeechMachineGameManager : MonoBehaviour
{
    ...
    private SpeechElements GetPlayableSpeechElement()
    {
        ...
        var randomSpeechElement = playableSpeechElements[randomIndex];
        
        if (Enum.TryParse(randomSpeechElement.Key, out SpeechElements speechElementEnum))
        {
            // Convert the enum to its underlying integer value
            int enumInt = (int)speechElementEnum;

            // Output the integer value
            Console.WriteLine($"The integer value of {randomSpeechElement.Key} is {enumInt}.");
        }
        else
        {
            // Handle the case where the string is not a valid enum name
            Console.WriteLine("Invalid enum name: " + randomSpeechElement.Key);
        }
